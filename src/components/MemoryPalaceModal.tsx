...
export const MemoryPalaceModal: React.FC<MemoryPalaceModalProps> = ({
  ...
  const [newNodeCategory, setNewNodeCategory] = useState<KnowledgeCategory>('Personal');
  const [newNodeTitle, setNewNodeTitle] = useState('');
  const [newNodeSummary, setNewNodeSummary] = useState('');
  const [newNodeDataPoints, setNewNodeDataPoints] = useState('');
  ...
  const filteredNodes = knowledgeNodes.filter(node => {
    ...
    const matchesSearch = !searchQuery || 
      ...
      node.dataPoints?.some(d => d.toLowerCase().includes(searchQuery.toLowerCase())) ||
  ...
  const handleAskMemory = async (q?: string) => {
    ...
    try {
      ...
      setMemoryAnswer({
        answer: `Unable to query context right now: ${err.message}`,
        relevantNodes: []
      });
    } finally {
      setIsAsking(false);
  ...
  const handleCreateNode = async (e: React.FormEvent) => {
    ...
    const newNode: KnowledgeNode = {
      ...
      dataPoints: newNodeDataPoints.split('\n').map(s => s.trim()).filter(Boolean),
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-xl font-bold text-slate-900">Knowledge & Context Hub</h2>
              ...
              <p className="text-xs text-slate-500">
                Captures concrete details, exact dates, preferences, and project facts from your conversations to ground your AI partner.
              </p>
            </div>
          </div>
          <button
        ...
        <div className="px-6 border-b border-slate-200 bg-slate-50/70 flex items-center justify-between">
          <div className="flex gap-4">
            ...
            >
              ...
              Context & Facts ({knowledgeNodes.length})
            </button>
            <button
            ...
            <div className="space-y-5">
                ...
                >
                  ...
                  <div>
                    <label className="block text-[11px] font-medium text-slate-700 mb-1">
                      Captured Facts & Data Points (one per line, e.g. "Birthday: September 4th", "Stack: TypeScript & React")
                    </label>
                    <textarea
                      value={newNodeDataPoints}
                      onChange={e => setNewNodeDataPoints(e.target.value)}
                      placeholder="Birthday: September 4th&#10;Role: Systems Architect&#10;Schedule: 8 AM - 11 AM Deep Work"
                      rows={2}
                      className="w-full text-xs p-2 bg-slate-50 border border-slate-200 rounded-lg"
                    />
                  </div>
              ...
              {filteredNodes.length === 0 ? (
                <div className="text-center py-12 bg-white rounded-xl border border-slate-200">
                  ...
                  <p className="text-sm font-semibold text-slate-700">No context nodes found</p>
                  <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1">
                    Brainstorm with your AI partner to capture concrete facts and dates automatically, or click "New Context Node".
                  </p>
                </div>
              ) : (
                    ...
                    return (
                      ...
                      >
                        <div>
                          ...
                          {/* Specific Captured Facts & Data Points */}
                          {node.dataPoints && node.dataPoints.length > 0 && (
                            <div className="space-y-1.5 mb-3 bg-indigo-50/70 p-2.5 rounded-lg border border-indigo-100/80">
                              <span className="text-[10px] font-bold uppercase tracking-wider text-indigo-700 flex items-center gap-1">
                                <Database className="w-3 h-3 text-indigo-500" />
                                Captured Facts & Data
                              </span>
                              <div className="flex flex-wrap gap-1.5 pt-0.5">
                                {node.dataPoints.map((dp, idx) => (
                                  <span
                                    key={idx}
                                    className="inline-flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-md bg-white border border-indigo-200 text-slate-800 shadow-2xs"
                                  >
                        ...
                        <div className="flex items-center justify-between text-[10px] text-slate-400 pt-2 border-t border-slate-100">
                          ...
                          <span className="text-emerald-600 font-medium">
                            ✓ Active in AI Context Grounding
                          </span>
                        </div>
                      </div>
                    );
            ...
            <div className="space-y-5">
              <div className="bg-gradient-to-r from-indigo-500 to-sky-600 text-white rounded-xl p-5 shadow-sm">
                <h3 className="text-base font-bold flex items-center gap-2">
                  ...
                  Query Context & Knowledge Hub
                </h3>
                <p className="text-xs text-indigo-100 mt-1 max-w-xl">
                  Query across your captured data points, specific dates, preferences, and project facts. Gemini answers strictly using your personal context.
                </p>
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  {[
                    "When is my birthday?",
                    "What is my primary tech stack?",
                    "What is my daily deep work schedule?",
                    "Summarize my active projects & deadlines"
                  ].map((chip, i) => (
                    <button
                      key={i}
                      onClick={() => {
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="Ask a question about your dates, preferences, tech stacks, or captured facts..."
              ...
              {isAsking && (
                <div className="p-8 text-center bg-white rounded-xl border border-slate-200">
                  ...
                  <p className="text-xs font-semibold text-slate-700">Searching your captured context and facts...</p>
                  <p className="text-[11px] text-slate-400 mt-1">Cross-referencing your personal dates, preferences, and journals...</p>
                </div>
              )}
